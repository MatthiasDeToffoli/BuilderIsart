...
com_isartdigital_perle_Main.prototype = $extend(EventEmitter.prototype,{
	...
	,forceImport: function() {
		var arrayClass = [com_isartdigital_perle_game_sprites_Ground,com_isartdigital_perle_game_sprites_Building];
		var lClassName;
...
com_isartdigital_perle_game_GameManager.prototype = {
	start: function() {
		...
		this.loadSave();
		com_isartdigital_perle_game_managers_CameraManager.centerCamera();
	...
	,loadSave: function() {
		var lSave = com_isartdigital_perle_game_managers_SaveManager.load();
		if(lSave != null) {
			com_isartdigital_perle_game_virtual_VCell.buildFromSave(lSave);
		} else {
			com_isartdigital_perle_game_virtual_VCell.buildInitial();
...
com_isartdigital_perle_game_managers_CameraManager.centerCamera = function() {
	var groundCenter = com_isartdigital_perle_game_iso_IsoManager.modelToIsoView(new PIXI.Point(20,40));
	com_isartdigital_perle_game_managers_CameraManager.target.y -= groundCenter.y / 2;
	com_isartdigital_perle_game_managers_CameraManager.target.x -= groundCenter.x / 2;
};
...
com_isartdigital_perle_game_managers_ClippingManager.update = function() {
...
com_isartdigital_perle_game_managers_ClippingManager.activateCells = function(pClippingRect) {
	var _this = com_isartdigital_perle_game_virtual_VCell.list;
	var tmp = new haxe_ds__$StringMap_StringMapIterator(_this,_this.arrayKeys());
	while(tmp.hasNext()) {
		var lClassArray = tmp.next();
		...
		while(_g1 < _g) {
			...
			while(_g3 < _g2) {
				var y = _g3++;
				if(lClassArray.h[x] != null && lClassArray.h[x].get(y) != null && !lClassArray.h[x].get(y).active) {
					lClassArray.h[x].get(y).activate();
				}
			}
		}
	}
	...
	while(tmp.hasNext()) {
		...
		while(_g1 < _g) {
			...
			while(_g3 < _g2) {
				var y = _g3++;
				if(lClassArray.h[x] != null && lClassArray.h[x].get(y) != null && lClassArray.h[x].get(y).active) {
					lClassArray.h[x].get(y).desactivate();
				}
			}
		}
	}
...
com_isartdigital_perle_game_managers_SaveManager.save = function() {
	var buildingSave = [];
	var groundSave = [];
	var tmp = com_isartdigital_perle_game_virtual_VCell.list.keys();
	while(tmp.hasNext()) {
		var lClassName = tmp.next();
		var _this = com_isartdigital_perle_game_virtual_VCell.list;
		var tmp1 = (__map_reserved[lClassName] != null?_this.getReserved(lClassName):_this.h[lClassName]).keys();
		while(tmp1.hasNext()) {
			var x = tmp1.next();
			var _this1 = com_isartdigital_perle_game_virtual_VCell.list;
			var tmp2 = (__map_reserved[lClassName] != null?_this1.getReserved(lClassName):_this1.h[lClassName]).get(x).keys();
			while(tmp2.hasNext()) {
				var y = tmp2.next();
				if(lClassName == "Building") {
					var _this2 = com_isartdigital_perle_game_virtual_VCell.list;
					buildingSave.push((__map_reserved[lClassName] != null?_this2.getReserved(lClassName):_this2.h[lClassName]).get(x).get(y).tileDesc);
				}
				if(lClassName == "Ground") {
					var _this3 = com_isartdigital_perle_game_virtual_VCell.list;
					groundSave.push((__map_reserved[lClassName] != null?_this3.getReserved(lClassName):_this3.h[lClassName]).get(x).get(y).tileDesc);
				}
			}
		}
	}
	com_isartdigital_perle_game_managers_SaveManager.currentSave = { ground : groundSave, building : buildingSave, COL_X_LENGTH : 20, ROW_Y_LENGTH : 40};
	js_Browser.getLocalStorage().setItem("com_isartdigital_myBuilderName",JSON.stringify(com_isartdigital_perle_game_managers_SaveManager.currentSave));
};
com_isartdigital_perle_game_managers_SaveManager.destroy = function() {
	js_Browser.getLocalStorage().setItem("com_isartdigital_myBuilderName",JSON.stringify(null));
};
com_isartdigital_perle_game_managers_SaveManager.load = function() {
	if(com_isartdigital_perle_game_managers_SaveManager.currentSave == null) {
		com_isartdigital_perle_game_managers_SaveManager.currentSave = JSON.parse(js_Browser.getLocalStorage().getItem("com_isartdigital_myBuilderName"));
		if(com_isartdigital_perle_game_managers_SaveManager.currentSave != null) {
			if(com_isartdigital_perle_game_managers_SaveManager.currentSave.COL_X_LENGTH != 20 || com_isartdigital_perle_game_managers_SaveManager.currentSave.ROW_Y_LENGTH != 40) {
				throw new js__$Boot_HaxeError("DIFFERENT VALUE Ground.COL_X_LENGTH or Ground.ROW_Y_LENGTH !! (use destroy())");
			}
		}
	}
...
com_isartdigital_perle_game_sprites_Tile.__super__ = com_isartdigital_utils_game_StateGraphic;
com_isartdigital_perle_game_sprites_Tile.prototype = $extend(com_isartdigital_utils_game_StateGraphic.prototype,{
	positionTile: function(pGridX,pGridY) {
		this.position = com_isartdigital_perle_game_iso_IsoManager.modelToIsoView(new PIXI.Point(pGridX,pGridY));
	}
...
com_isartdigital_perle_game_sprites_Building.createBuilding = function(pTileDesc) {
	...
	lBuilding.positionTile(pTileDesc.mapX,pTileDesc.mapY);
	var tmp = new PIXI.Point(pTileDesc.mapX,pTileDesc.mapY);
...
com_isartdigital_perle_game_sprites_Building.prototype = $extend(com_isartdigital_perle_game_sprites_Tile.prototype,{
	...
	,doActionPhantom: function() {
		...
		this.position = com_isartdigital_perle_game_iso_IsoManager.modelToIsoView(bestMapPos);
		...
		this.precedentBesMapPos.copy(bestMapPos);
	}
	...
	,buildingOnGround: function() {
		if(this.colMin < 0 || this.rowMin < 0 || this.colMax >= 20 || this.rowMax >= 40) {
			return false;
		}
		return true;
	}
	,buildingCollideOther: function() {
		var _g1 = 0;
		var _g = com_isartdigital_perle_game_managers_SaveManager.currentSave.building.length;
		while(_g1 < _g) if(this.collisionRectDesc(com_isartdigital_perle_game_managers_SaveManager.currentSave.building[_g1++])) {
			return false;
		}
		return true;
	}
	,collisionRect: function(rect2) {
		if(this.colMin < rect2.colMax + 1 && this.colMax + 1 > rect2.colMin && this.rowMin < rect2.rowMax + 1) {
			return this.rowMax + 1 > rect2.rowMin;
		} else {
			return false;
		}
	}
	,collisionRectDesc: function(pDesc) {
		var tmp;
		var tmp1;
		var tmp2 = this.colMin;
		var tmp3 = pDesc.mapX;
		var _this = com_isartdigital_perle_game_sprites_Building.ASSETNAME_TO_MAPSIZE;
		var key = pDesc.assetName;
		if(tmp2 < tmp3 + (__map_reserved[key] != null?_this.getReserved(key):_this.h[key]).width) {
			tmp1 = this.colMax + 1 > pDesc.mapX;
		} else {
			tmp1 = false;
		}
		if(tmp1) {
			var tmp4 = this.rowMin;
			var tmp5 = pDesc.mapY;
			var _this1 = com_isartdigital_perle_game_sprites_Building.ASSETNAME_TO_MAPSIZE;
			var key1 = pDesc.assetName;
			tmp = tmp4 < tmp5 + (__map_reserved[key1] != null?_this1.getReserved(key1):_this1.h[key1]).height;
		} else {
			tmp = false;
		}
		if(tmp) {
			return this.rowMax + 1 > pDesc.mapY;
		} else {
			return false;
		}
	}
	,newBuild: function() {
		var vBuilding = new com_isartdigital_perle_game_virtual_VBuilding({ className : "Building", assetName : this.assetName, mapX : this.colMin, mapY : this.rowMin});
		vBuilding.activate();
		vBuilding = null;
		com_isartdigital_perle_game_sprites_Building.removePhantom();
		com_isartdigital_perle_game_managers_SaveManager.save();
	}
	,setMapColRow: function(pMapPos,pMapSize) {
		this.colMax = js_Boot.__cast(pMapPos.x + pMapSize.width - 1 , Int);
		this.colMin = js_Boot.__cast(pMapPos.x , Int);
		this.rowMax = js_Boot.__cast(pMapPos.y + pMapSize.height - 1 , Int);
		this.rowMin = js_Boot.__cast(pMapPos.y , Int);
	}
...
com_isartdigital_perle_game_sprites_Ground.createGround = function(pTileDesc) {
	...
	com_isartdigital_perle_game_sprites_Ground.mapArray[pTileDesc.mapX][pTileDesc.mapY] = lGround;
	lGround.positionTile(pTileDesc.mapX,pTileDesc.mapY);
	lGround.init();
...
com_isartdigital_perle_game_sprites_Ground.lightCell = function(pPoint) {
	var lGround;
	if(com_isartdigital_perle_game_sprites_Ground.mapArray[Math.floor(pPoint.x)] == null) {
	...
	lGround = com_isartdigital_perle_game_sprites_Ground.mapArray[Math.floor(pPoint.x)][Math.floor(pPoint.y)];
	if(lGround != null) {
	...
	} else if(lGround == null && com_isartdigital_perle_game_sprites_Ground.previousCell != null) {
		com_isartdigital_perle_game_sprites_Ground.previousCell.removeFilterCell();
	}
};
...
com_isartdigital_perle_game_sprites_Ground.prototype = $extend(com_isartdigital_perle_game_sprites_Tile.prototype,{
	...
	,recycle: function() {
		com_isartdigital_perle_game_sprites_Ground.mapArray[this.mapX][this.mapY] = null;
		com_isartdigital_perle_game_sprites_Tile.prototype.recycle.call(this);
	}
	,destroy: function() {
		com_isartdigital_perle_game_sprites_Ground.mapArray[this.mapX][this.mapY] = null;
...
var com_isartdigital_perle_game_virtual_VCell = function(pDescription) {
	this.active = false;
	this.tileDesc = pDescription;
	this.position = com_isartdigital_perle_game_iso_IsoManager.modelToIsoView(new PIXI.Point(this.tileDesc.mapX,this.tileDesc.mapY));
	this.addToList();
};
$hxClasses["com.isartdigital.perle.game.virtual.VCell"] = com_isartdigital_perle_game_virtual_VCell;
com_isartdigital_perle_game_virtual_VCell.__name__ = ["com","isartdigital","perle","game","virtual","VCell"];
com_isartdigital_perle_game_virtual_VCell.buildInitial = function() {
	var _g = 0;
	while(_g < 20) {
		var x = _g++;
		var _g1 = 0;
		while(_g1 < 40) {
			var y = _g1++;
			new com_isartdigital_perle_game_virtual_VGround({ className : "Ground", assetName : com_isartdigital_perle_game_virtual_VCell.ROAD_MAP[x] != null && com_isartdigital_perle_game_virtual_VCell.ROAD_MAP[x][y] != null && com_isartdigital_perle_game_virtual_VCell.ROAD_MAP[x][y] != ""?com_isartdigital_perle_game_virtual_VCell.ROAD_MAP[x][y]:"Ground", mapX : x, mapY : y});
		}
	}
};
com_isartdigital_perle_game_virtual_VCell.buildFromSave = function(pSave) {
	...
	var _g11 = 0;
	var _g2 = lLength;
	while(_g11 < _g2) new com_isartdigital_perle_game_virtual_VBuilding(pSave.building[_g11++]);
};
com_isartdigital_perle_game_virtual_VCell.prototype = {
	removeLink: function() {
	...
	,addToList: function() {
		...
		var _this = com_isartdigital_perle_game_virtual_VCell.list;
		var key = this.tileDesc.className;
		if((__map_reserved[key] != null?_this.getReserved(key):_this.h[key]) == null) {
			var v = new haxe_ds_IntMap();
			var _this1 = com_isartdigital_perle_game_virtual_VCell.list;
			var key1 = this.tileDesc.className;
			if(__map_reserved[key1] != null) {
				_this1.setReserved(key1,v);
			} else {
				_this1.h[key1] = v;
			}
		}
		var _this2 = com_isartdigital_perle_game_virtual_VCell.list;
		var key2 = this.tileDesc.className;
		if((__map_reserved[key2] != null?_this2.getReserved(key2):_this2.h[key2]).get(xRow) == null) {
			var _this3 = com_isartdigital_perle_game_virtual_VCell.list;
			var key3 = this.tileDesc.className;
			var this1 = __map_reserved[key3] != null?_this3.getReserved(key3):_this3.h[key3];
			var v1 = new haxe_ds_IntMap();
			this1.set(xRow,v1);
		}
		var _this4 = com_isartdigital_perle_game_virtual_VCell.list;
		var key4 = this.tileDesc.className;
		(__map_reserved[key4] != null?_this4.getReserved(key4):_this4.h[key4]).get(xRow).set(yCol,this);
	}
	,linkVirtual: function() {
		this.myInstance.linkVirtualCell(this);
	}
	,__class__: com_isartdigital_perle_game_virtual_VCell
};
var com_isartdigital_perle_game_virtual_VBuilding = function(pDescription) {
	com_isartdigital_perle_game_virtual_VCell.call(this,pDescription);
};
...
com_isartdigital_perle_game_virtual_VBuilding.__super__ = com_isartdigital_perle_game_virtual_VCell;
com_isartdigital_perle_game_virtual_VBuilding.prototype = $extend(com_isartdigital_perle_game_virtual_VCell.prototype,{
	activate: function() {
		com_isartdigital_perle_game_virtual_VCell.prototype.activate.call(this);
		this.myInstance = com_isartdigital_perle_game_sprites_Building.createBuilding(this.tileDesc);
		this.linkVirtual();
	}
...
com_isartdigital_perle_game_virtual_VGround.__super__ = com_isartdigital_perle_game_virtual_VCell;
com_isartdigital_perle_game_virtual_VGround.prototype = $extend(com_isartdigital_perle_game_virtual_VCell.prototype,{
	activate: function() {
		com_isartdigital_perle_game_virtual_VCell.prototype.activate.call(this);
		this.myInstance = com_isartdigital_perle_game_sprites_Ground.createGround(this.tileDesc);
		this.linkVirtual();
	}
...
com_isartdigital_perle_ui_hud_ButtonBuild.prototype = $extend(com_isartdigital_utils_ui_Button.prototype,{
	_mouseDown: function(pEvent) {
		...
		com_isartdigital_perle_game_sprites_Building.onClickHudBuilding(this.assetName.substring(this.assetName.indexOf("Button"),-1));
	}
...
var com_isartdigital_perle_ui_hud_Hud = function() {
	...
	this.BUTTONS_NAMES = ["FactoryButton","HouseButton","TreesButton","VillaButton"];
	com_isartdigital_utils_ui_Screen.call(this);
	this.set_modal(false);
	this.hudBottom = new PIXI.Container();
	this.buildingBackground = new com_isartdigital_perle_ui_UIElement("Hud");
	this.hudBottom.addChild(this.buildingBackground);
	...
	createButtons: function(pContainer,pNames,pStartPos,pMargin) {
		...
		while(_g1 < _g) {
			...
			if(pNames[i] == pNames[0]) {
				...
				var key2 = pNames[i];
				var tmp1 = __map_reserved[key2] != null?_this2.getReserved(key2):_this2.h[key2];
				tmp1.x += pStartPos.x;
			} else {
				var _this3 = this.buttonMap;
				var key3 = pNames[i];
				var tmp2 = __map_reserved[key3] != null?_this3.getReserved(key3):_this3.h[key3];
				var _this4 = this.buttonMap;
				var key4 = pNames[0];
				tmp2.y = (__map_reserved[key4] != null?_this4.getReserved(key4):_this4.h[key4]).y;
				var _this5 = this.buttonMap;
				var key5 = pNames[i];
				var tmp3 = __map_reserved[key5] != null?_this5.getReserved(key5):_this5.h[key5];
				var _this6 = this.buttonMap;
				var key6 = pNames[i - 1];
				var tmp4 = (__map_reserved[key6] != null?_this6.getReserved(key6):_this6.h[key6]).x;
				var _this7 = this.buttonMap;
				var key7 = pNames[i - 1];
				tmp3.x = tmp4 + (__map_reserved[key7] != null?_this7.getReserved(key7):_this7.h[key7]).width + pMargin;
			}
			var _this8 = this.buttonMap;
			var key8 = pNames[i];
			pContainer.addChild(__map_reserved[key8] != null?_this8.getReserved(key8):_this8.h[key8]);
		}
	}
	,destroy: function() {
...
var com_isartdigital_utils_ui_smart_TextSprite = function(pData) {
	...
	var lTxt = new PIXI.Text(com_isartdigital_utils_ui_smart_TextSprite.parseText(StringTools.replace(pData.text,"</BR>","\r")),lStyle);
...
com_isartdigital_utils_ui_smart_UIBuilder.build = function(pId,pFrame) {
	...
	while(_g1 < _g) {
		...
		if(lKeyFrame.symbol.data != null && lKeyFrame.symbol.data.className != null) {
			...
			lObj = new pixi_flump_Movie(lKeyFrame.symbolId);
...
com_isartdigital_perle_game_managers_SaveManager.SAVE_NAME = "com_isartdigital_myBuilderName";
...
com_isartdigital_perle_game_sprites_Ground.COL_X_LENGTH = 20;
com_isartdigital_perle_game_sprites_Ground.ROW_Y_LENGTH = 40;
com_isartdigital_perle_game_sprites_Ground.FILTER_BRIGHTNESS = 1.3;
com_isartdigital_perle_game_sprites_Ground.mapArray = (function($this) {
	var $r;
	var _g = [];
	{
		var _g1 = 0;
...
com_isartdigital_perle_game_virtual_VCell.ROAD_MAP = [["","","","","Road_br","Road_tl","",""],["","","","Road_br","Road_tl","","",""],["Road_v","Road_v","Road_v","Road_c","","","",""],["","","","Road_h","","","",""],["Road_v","Road_v","Road_v","Road_tl","","","",""],["","","","","","","",""],["","","","","","","",""],["","","","","","","",""],["","","","","","","",""],["","","","","","","",""],["","","","","","","",""],["","","","","","","",""]];
com_isartdigital_perle_game_virtual_VCell.list = new haxe_ds_StringMap();
