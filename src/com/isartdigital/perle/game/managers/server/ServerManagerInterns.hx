...
typedef EventErrorIntern = {
	@optionnal var errorID:Int;
	var idIntern:Int;
	@optionnal var price:Int;
}
...
class ServerManagerInterns
{
	...
	public static function execute(pAction:DbAction, ?internId:Int=null, ?eventId:Int=null):Void {
		...
		switch (pAction) {
			case DbAction.ADD:
				if (internId != null) 
					ServerManager.callPhpFile(onSuccessAddIntern, onErrorBuyIntern, ServerFile.MAIN_PHP, [
						ServerManager.KEY_POST_FILE_NAME => ServerFile.INTERNS,
						"funct" => actionCall,
						"idInt" => internId
					]);
			case DbAction.REM:
				if (internId != null) ServerManager.callPhpFile(onSucessRemoveIntern, onErrorRemoveIntern, ServerFile.MAIN_PHP, [
						ServerManager.KEY_POST_FILE_NAME => ServerFile.INTERNS,
						"funct" => actionCall,
						"idInt" => internId
					]);
			case DbAction.UPDT:
				if (internId != null) ServerManager.callPhpFile(onDataCallback, onErrorCallback, ServerFile.MAIN_PHP, [
						ServerManager.KEY_POST_FILE_NAME => ServerFile.INTERNS,
						"funct" => actionCall,
						"idInt" => internId,
						"str" => Intern.getIntern(internId).stress
					]);
			...
			case DbAction.GET_SPE_JSON:
				ServerManager.callPhpFile(Intern.getPlayerInterns, onErrorCallback, ServerFile.MAIN_PHP, [
					ServerManager.KEY_POST_FILE_NAME => ServerFile.INTERNS,
					"funct" => actionCall
				]);
			default: return;
	...
	public static function onSuccessAddIntern(object:Dynamic):Void {
		if (object.charAt(0) == "{") {
			...
			if (object != null && object > 0) ResourcesManager.spendTotal(GeneratorType.soft, object);
		}
	}	
