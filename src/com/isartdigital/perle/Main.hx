...
class Main extends EventEmitter
{
	...
	public static inline var ONLY_LD_FOR_SMARTPHONE:Bool = true; //Change to false for md/hd on smartphone
	...
	private function preloadAssets(pLoader:Loader):Void {
		...
		else if (ONLY_LD_FOR_SMARTPHONE){
		...
		GameStage.getInstance().init(render, 2048, 1366, true, true, true, true, true, true, true,true, true); // premier false => éviter le 0,0 au centre.
	...
	private function loadAssets (pLoader:GameLoader): Void {
		...
		lLoader.addAssetFile(UI_FOLDER + DeviceCapabilities.textureType+"/DailiesSocials/library.json");
		...
		lLoader.addAssetFile(UI_FOLDER + DeviceCapabilities.textureType+"/CreditsPage/library.json");	
		lLoader.addAssetFile(IN_GAME_FOLDER + DeviceCapabilities.textureType+"/title_BG_placeholder/library.json");
